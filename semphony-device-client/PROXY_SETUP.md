...
This guide explains how to set up the Semphony device client in a proxy/relay configuration where one PC (PC2) has internet connectivity and another PC (PC1) is only connected to the local network.

## Architecture Overview

The proxy setup uses a **local LAN relay** feature built into the Semphony device client:
...
python3 -m device_client.tools.pc1_simulator --host 192.168.1.100
```
...
For more details, see `device_client/relay_gateway.py`.

...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="min-h-screen bg-white">
        <flux:sidebar sticky collapsible="mobile" class="border-e border-zinc-200 bg-zinc-50">
            <flux:sidebar.header>
                <x-app-logo class="w-full" :sidebar="true" href="{{ route('dashboard') }}" wire:navigate />
            ...
            <flux:sidebar.nav>
                <flux:sidebar.group :heading="__('Platform')" class="grid">
                    ...
                    <flux:sidebar.item icon="server" :href="route('systems.index')" :current="request()->routeIs('systems.*')" wire:navigate>
                        {{ __('Systems') }}
                    </flux:sidebar.item>

                    <flux:sidebar.item icon="monitor" :href="route('clients.index')" :current="request()->routeIs('clients.*')" wire:navigate>
                        {{ __('Clients') }}
                    </flux:sidebar.item>
